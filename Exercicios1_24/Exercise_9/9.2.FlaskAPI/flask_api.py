...
# POST /insert_db - Accepts JSON and inserts data into the database
...
def insert_data_db():
    ...
    try:
        ...
        return jsonify({"message": "Dados inseridos com sucesso na base de dados"}), 201
    
    except Exception as erro:
        print(f"ERROR: Erro ao tentar inserir os dados na base de dados: {erro}")
        connection.rollback()
        return jsonify({"error": "Erro ao inserir os dados na base de dados"}), 500
    
    finally:
        connection.close()

# POST /create_file - Accepts JSON and writes it to a file
...
def create_file():
    ...
    try:
        ...
        return jsonify({"message": "Arquivo JSON criado com sucesso"}), 201
    
    except Exception as erro:
        print(f"ERROR: Erro ao criar o arquivo JSON: {erro}")
        return jsonify({"error": "Erro ao criar o arquivo JSON"}), 500
